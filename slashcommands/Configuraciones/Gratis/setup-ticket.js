const Discord = require("discord.js"); //Requerimos los paquetes
const {
	EmbedBuilder,
	ButtonStyle,
	ButtonBuilder,
...
module.exports = {
	...
	options: [
		{
			name: "on",
			description:
				"Habilita el sistema de ticket donde se habriran los tickets",
			type: 1,
			options: [
				{
					name: "categoria",
	...
	async execute(_, interaction) {
		...
		const nombreboton =
			interaction.options.getString("nombre_boton") || "Tickets";
		const emojiboton = interaction.options.getString("emoji_boton") || "✅";
		const colorboton =
			interaction.options.getString("color_boton") ||
		...
		if (action === "on") {
			if (data)
				return interaction.reply({
					content: "Este servidor ya tiene activado el sistema de tickets",
			...
			interaction.reply({
				content: `El sistema de ticket fue creado correctamente en el canal ${canalId}`,
			});
		...
		if (action === "edit") {
			if (!data)
				return interaction.reply({
					content: "Este servidor no tiene habilitado el sistema de tickets",
					flags: MessageFlagsBits.Ephemeral,
				});
			...
			interaction.reply({
				content: "El sistema de ticket ha sido editado con exito",
			});
		}
		if (action === "off") {
			if (!data)
			...
			const _toDelete = await TicketSetup.findOne({
				ServerID: interaction.guild.id,
			});
			if (_toDelete) await _toDelete.deleteOne();

			interaction.reply({
				content: "Los sistemas de tickets han sido deshabilitados",
				flags: MessageFlagsBits.Ephemeral,
			});
		}
	},
