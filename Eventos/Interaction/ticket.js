...
const MessageFlagsBits = Discord.MessageFlagsBits ?? { Ephemeral: 64 };
	...
	async execute(interaction, client) {
		if (interaction.isButton()) {
			if (interaction.customId === "crear") {
				try {
					...
					const razon_txt = new TextInputBuilder()
						.setCustomId("razon")
						.setLabel("Asunto")
					...
					const data_ticket = await TicketCanal.find({
						ServerID: interaction.guild.id,
					...
					for (const ticket of data_ticket) {
						if (interaction.guild.channels.cache.get(ticket.Canal))
							return modalInteraction.reply({
								content: `❌ Ya tienes un ticket habierto: <#${ticket.Canal}>`,
					...
					const mensaje = new EmbedBuilder()
						...
						.setDescription(
							`Ticket habierto por <@${interaction.user.id}>\nAsunto ${razon2}`,
						)
						.setColor("Green");
		...
		if (interaction.customId === "cerrar") {
			try {
				...
				const data_ticket = await TicketCanal.findOne({
					ServerID: interaction.guild.id,
					Canal: interaction.channel.id,
				});

				if (data_ticket?.Cerrado)
					return interaction.reply({
						content: `Ticket cerrado por <@${interaction.user.id}>`,
					});
				...
				const verificar = await interaction.channel.send({
					content: "📌 ¿Estás seguro que quieres cerrar tu ticket?",
					components: [btn1],
				});
				...
				collector.on("collect", (boton) => {
					if (boton.user.id !== interaction.user.id)
						return boton.reply({
							content: `Solo ${interaction.user} puede usar ese boton`,
							flags: MessageFlagsBits.Ephemeral,
						});
				...
				collector.on("end", (collected) => {
					if (collected?.first()?.customId) {
						verificar.edit({ components: [btn2] });
					} else {
						verificar.edit({
							embeds: [verificar.embeds[0]],
							components: [btn3],
						});
					}
				});
		...
		if (interaction.customId === "borrar") {
			try {
				...
				if (!permiso)
					return interaction.reply({
						content: "❌ No tienes suficientes permiso",
						flags: MessageFlagsBits.Ephemeral,
					});
		...
		if (interaction.customId === "siboton") {
			try {
				...
				const mensaje = new EmbedBuilder()
					.setDescription(
						"✅ El ticket sera eliminado dentro de **5** segundos",
					)
					.setColor("Green");
				return (
					interaction.reply({ embeds: [mensaje], flags: MessageFlagsBits.Ephemeral }) &&
					setTimeout(() => {
		...
		if (interaction.customId === "noboton") {
			...
			await interaction.reply({ embeds: [mensaje], flags: MessageFlagsBits.Ephemeral });
			} catch (error) {
				console.log(error);
			}
		}
